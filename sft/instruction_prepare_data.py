...
def get_ids(instruction_dataset_repo_name, examples_list, tokenizer, split):
    ...
    for example in tqdm(examples_list, desc="Processing examples"):
        ...
        compressed_chunk_text = llm_lingua.compress_prompt(org_text, rate=0.2)["compressed_prompt"]
...
def get_examples(model_id, instruction_dataset_repo, samples_num, min_len, max_len, dataset_repo):
    ...
    instruction_dataset_repo_name = instruction_dataset_repo.split('/')[-1]
    train_data_name = f"output/{instruction_dataset_repo_name}_train_"+model_name+f"_{samples_num}samples_instruction_rarc_lingua2_5x.pt"
    eval_data_name = f"output/{instruction_dataset_repo_name}_eval_"+model_name+f"_{samples_num}samples_instruction_rarc_lingua2_5x.pt"
