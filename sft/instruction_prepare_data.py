...
def parse_args():
    ...
    parser.add_argument('--work_dir', type=str, required=False, default="../experiment/rebuttal/15x_8gpu/SAC_Long",help='Directory including the configuration file')
...
def get_examples_list(instruction_dataset_repo, split):
    ...
    if os.path.exists(f'output/{instruction_dataset_repo_name}_{split}_instruction_dataset.jsonl'):
        with open(f'output/{instruction_dataset_repo_name}_{split}_instruction_dataset.jsonl', 'r', encoding='utf-8') as f:
            for line in f:
